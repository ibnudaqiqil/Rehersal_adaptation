...
def train(model, device, train_set_loader, optimizer, epoch, logging_interval=100):
    model.train()
    for batch_idx, (data, target,_) in enumerate(train_set_loader):
...
class SpikingNeuronLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        inner_excitation = inner_excitation - \
            (self.penalty_threshold/self.threshold *
...
class SpikingNeuralNetwork(nn.Module):
    ...
    def visualize_all_neurons(self, x):
        ...
        for i, (all_layer_states, all_layer_outputs) in enumerate(layers_state):
            layer_state = torch.stack(
                all_layer_states).data.cpu().numpy().squeeze().transpose()
            layer_output = torch.stack(
                all_layer_outputs).data.cpu().numpy().squeeze().transpose()

            self.plot_layer(
                layer_state, title="Inner state values of neurons for layer {}".format(i))
            self.plot_layer(
                layer_output, title="Output spikes (activation) values of neurons for layer {}".format(i))
    ...
    def visualize_neuron(self, x, layer_idx, neuron_idx):
        ...
        self.plot_neuron(layer_state[neuron_idx], title="Inner state values neuron {} of layer {}".format(
            neuron_idx, layer_idx))
        self.plot_neuron(layer_output[neuron_idx], title="Output spikes (activation) values of neuron {} of layer {}".format(
            neuron_idx, layer_idx))
...
'''
spiking_model = SpikingNeuralNetwork(device, n_time_steps=128, begin_eval=0)
...
for task_id, train_taskset in enumerate(scenario):
    ...
    print(f"[red]Task {task_id} jumlah kelas = {jumlah_kelas}")
    #prepare datatrain set
    train_taskset, val_taskset = split_train_val(train_taskset, val_split=0.1)
    train_loader = DataLoader(
        train_taskset, batch_size=BATCH_SIZE, shuffle=False)
